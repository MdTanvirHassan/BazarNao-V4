...
class HomeController extends Controller
{
    ...
    public function all_flash_deals()
    {
        ...
        if (count($data) > 0) {
            ...
            if ($flashstart <= $todaytime && $flashend >= $todaytime) {
                ...
                flash("No happy hours avaiable in this time")->success();
                return back();
            }
        } else {
            flash("No happy hours avaiable in this time")->success();
    ...
    public function staff_dashboard()
    {
        ...
        if (auth()->user()->staff->role->name == 'Sales Executive') {
            ...
            $data['total_purchase_qty'] = Purchase_order::leftjoin('purchase_order_item', 'purchase_order.id', 'purchase_order_item.po_id')
                ->whereBetween('purchase_order.created_at', [$from_date, $to_date])
                ->where('created_by', Auth::user()->id)
                ->where('status', '2')->select('purchase_order_item.qty')->sum('qty');

            $data['total_purchase_amount'] = Purchase_order::leftjoin('purchase_order_item', 'purchase_order.id', 'purchase_order_item.po_id')
                ->whereBetween('purchase_order.created_at', [$from_date, $to_date])
                ->where('created_by', Auth::user()->id)
            ...
            $data['daily_activities'] = Dailyactivity::where('user_id', Auth::user()->id)->where('date', date('Y-m-d'))->get();
            $data['purchasedetails'] = Purchase_order::where('purchase_order.status', 2)->where('purchase_order.created_by', Auth::user()->id)->where('date', date('Y-m-d'))
            ...
            $data['total_purchase_qty'] = Purchase_order::leftjoin('purchase_order_item', 'purchase_order.id', 'purchase_order_item.po_id')
                ->where('purchase_order.created_by', Auth::user()->id)
                ->whereBetween('purchase_order.created_at', [$from_date, $to_date])
            ...
            $data['daily_activities'] = Dailyactivity::where('user_id', Auth::user()->id)->where('date', date('Y-m-d'))->get();
            $data['purchasedetails'] = Purchase_order::where('purchase_order.status', 2)
                ->where('purchase_order.created_by', Auth::user()->id)
            ...
            $data['order_daily_activities'] = Order::where('delivery_boy', Auth::user()->id)
                ...
                ->leftJoin('order_details', 'orders.id', '=', 'order_details.order_id')
                ->groupBy('orders.id')
                ->select('orders.code', 'orders.delivery_boy', 'orders.warehouse', 'orders.grand_total', 'orders.id', 'orders.user_id', 'orders.guest_id', 'orders.shipping_address', 'orders.cash_collection', 'customers.customer_id', 'areas.name as areaname', 'order_details.delivery_status')->get();
        ...
        foreach ($request->phone as $key => $each) {
            ...
            if (auth()->user()->staff->role->name == 'Purchase Executive' || auth()->user()->staff->role->name == 'Purchase Manager') {
                ...
                if (!empty($request->paid_amount[$key])) {
                    ...
                    $purchase = Purchase_order::findOrFail($request->purchase_id[$key]);
   ...
   (select sum(deel.debit-deel.credit)where deel.date between '" . $start_date . "' and '" . $end_date . "' or deel.date is null) as due
    ...
    {
        $purchasedetails = Purchase_order::where('purchase_no', $request->purchaseno)
            ->rightjoin('suppliers', 'suppliers.supplier_id', 'purchase_order.supplier_id')
            ->select('suppliers.supplier_id', 'suppliers.name', 'suppliers.address', 'suppliers.phone', 'purchase_order.payment_amount', 'purchase_order.total_value')->first();
    ...
    public function purchase_list_new(Request $request)
    {
        ...
        if (!empty($start_date) && !empty($end_date) && empty($expiry_date_start) && empty($expiry_date_end)) {
            ...
            $data = Purchase_order::leftjoin('suppliers', 'suppliers.supplier_id', '=', 'purchase_order.supplier_id')
                ->orderBy('purchase_order.date', 'asc')
        ...
        if (empty($start_date) && empty($end_date) && !empty($expiry_date_start) && !empty($expiry_date_end)) {
            ...
            $data = Purchase_order::leftjoin('suppliers', 'suppliers.supplier_id', '=', 'purchase_order.supplier_id')
                ...
                ->orderBy('purchase_order.date', 'asc')
                ->whereIn('purchase_order.wearhouse_id', $warehousearray)
                ->select('purchase_order.*', 'suppliers.name', 'purchase_order_item.expiry_date');
            ...
            if (!empty($request->expiry_date_start) && !empty($request->expiry_date_end)) {
                ...
                $data = $data->whereBetween('purchase_order_item.expiry_date', [$expiry_date_start, $expiry_date_end]);
            } else {
                $data = $data->whereBetween('purchase_order_item.expiry_date', [$expiry_date_start, $expiry_date_end]);
            }
    ...
    public function purchase_list(Request $request)
    {
        ...
        return view('backend.staff_panel.purchase_manager.purchase_list', compact('data', 'sort_by', 'start_date', 'end_date'));
    }
    ...
    public function purchase_list_for_purchase_executive(Request $request)
    {
        ...
        $all_purchase = Purchase_order::leftjoin(
            'suppliers',
            ...
            ->select(
                ...
                'purchase_order.status as purstatus',
