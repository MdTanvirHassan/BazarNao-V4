...
class CartController extends Controller
{
        ...
        public function add(Request $request)
        {
            ...
            if (intval($product->max_qty) < $total) { 
                Cart::updateOrCreate([
                        'user_id' => $request->user_id,
                        ...
                        'tax' => $tax,
                        'shipping_cost' => 0,
                        'quantity' => DB::raw("$product->max_qty")
                    ]);
                    return response()->json([
                        'result' => false,
                        'cartQty'=>Cart::where('user_id',$request->user_id)->count(),
                        'message' => 'You Can not add more than '.($product->max_qty).' Quantity for this product'
                    ]);
                }
                else{
                        Cart::updateOrCreate([
