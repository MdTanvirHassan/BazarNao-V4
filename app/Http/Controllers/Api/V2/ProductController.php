...
class ProductController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if ($min != null && $min != "" && is_numeric($min)) {
            $products = $products->where('app_unit_price', '>=', $min);
        }

        if ($max != null && $max != "" && is_numeric($max)) {
            $products = $products->where('app_unit_price', '<=', $max);
        }

        switch ($sort_by) {
            case 'price_low_to_high':
                $products = $products->orderBy('app_unit_price', 'asc');
                break;

            case 'price_high_to_low':
                $products = $products->orderBy('app_unit_price', 'desc');
                break;
    ...
    public function discountProduct(Request $request){  
        if ($request->name != "" || $request->name != null) {
            ...
            ->whereRaw('published = 1 and discount > 0 and outofstock = 0')->where('products.name', 'like', '%'.$name.'%')->orWhere('product_translations.name', 'like', '%'.$name.'%')->groupBy('products.id')->select('products.*')
            ->paginate(15));
        }else{
            return new ProductMiniCollection(Product::whereRaw('published = 1 and discount > 0 and outofstock = 0')->paginate(15));
        }
    }
}
