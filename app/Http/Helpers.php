...
if (! function_exists('home_price')) {
    ...
    {
        $product = Product::findOrFail($id);
        $lowest_price = $product->app_unit_price;
        $highest_price = $product->app_unit_price;
...
function getShippingCost($index){
    ...
    foreach (Session::get('cart') as $key => $cartItem) {
        ...
        $calculate_total+=($product->app_unit_price*$cartItem['quantity']);
...
function generate_po_id($bill_id) {
    ...
    $date = date('Y');
    $temp_data = DB::table('purchase_order')
