    ...
    @if(Session::has('cart'))
        @if(count($cart = Session::get('cart')) > 0)
                ...
                @foreach($cart as $key => $cartItem)
                    ...
                    @if ($product != null)
                        <li class="list-group-item">
                            ...
                            <div style="width: 10%;">
                            <button onclick="updateQuantityP({{ $key }}, {{ $cartItem['quantity'] }},{{$cartItem['id']}})" class="btn btn-sm btn-icon " style="padding-left: 0px;">
                                        <i class="la la-angle-up"></i>
                                    </button><br/>
                                    <span id="rightqty_{{$cartItem['id']}}" style="padding-left: 8px;">{{ $cartItem['quantity'] }}</span><br/>
                                    <button onclick="updateQuantityM({{ $key }}, {{ $cartItem['quantity'] }},{{$cartItem['id']}})"  @php if($cartItem['quantity'] == 1) echo 'disabled ' @endphp onclick="minuscartqty({{ $key }})" class="btn btn-sm btn-icon" style="padding-left: 0px;">
            ...
            <div class="px-3 py-2 fs-15 border-top d-flex justify-content-between">
                <span class="opacity-60">{{translate('Offer Discount')}}</span>
                <span class="fw-600">{{ single_price(Session::get('offer_discount')) }}</span>
            </div>
            @endif
            <div class="px-3 py-2 fs-15 border-top d-flex justify-content-between">
