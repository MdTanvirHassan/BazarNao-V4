...
<script type="text/javascript">
    ...
    $('#barcode').keypress(function (event){
            setTimeout(() =>{
                ...
                var wearhouse_id = $('#wearhouse_id').val();
                    if(wearhouse_id === '' || wearhouse_id === undefined || wearhouse_id === null){
                        AIZ.plugins.notify('danger', 'Please select wearhouse First');
                        $('#barcode').val('');
                        return false;
                    }
                    
                $.post("{{ route('purchase.withbarcode') }}",{_token:AIZ.data.csrf, barcode:barcode,wearhouse_id:wearhouse_id}, function(data){
            if(data.success == 1){
                var row = $('#item_table').find('tbody>tr').length;
                row++;
                var str = "<tr id='row_" + row + "'>";
                ...
                data.products.forEach(function(product) {
                    ...
                    str += '<option data-qty="' + product.current_stock + '" data-price="' + product.purchase_price + '" value="' + product.id + '" ' + (isSelected ? 'selected' : '') + '>' + product.name + '</option>';
                });
