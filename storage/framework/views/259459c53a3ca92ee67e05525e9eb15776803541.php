...
src="https://www.facebook.com/tr?id=932980007684341&ev=PageView&noscript=1"
/></noscript>
<!-- End Meta Pixel Code -->
<?php endif; ?>
    ...
    $('.pro_list_block').removeAttr("style"); 
      ...
      $('#searchm').on('keyup', function(){
        ...
        function updateNavCart(){
            ...
            $.post('<?php echo e(route('cart.updateRightCart')); ?>', {_token: AIZ.data.csrf }, function(data){
        ...
        function directAdd(id,checking=false){
                ...
                $.ajax({
                   ...
                   success: function(data){
                    ...
                    $('#productlist_'+id).parents('button').prev('button').attr('onclick','updateQuantityM('+key+', '+(proqty+1)+','+id+')');
                    $('#productlist_'+id).parents('button').next('button').attr('onclick','updateQuantityP('+key+', '+(proqty+1)+','+id+')');
                            updateNavCart();
                            //cartAnimation(id);
                          //  $('#cart_items_sidenav').html(parseInt($('#cart_items_sidenav').html())+1);
                       }else{
...
function updateQuantityP(key, element,id){
    
        $.post('<?php echo e(route('cart.updateQuantity')); ?>', { _token:'<?php echo e(csrf_token()); ?>', key:key, quantity: (element+1)}, function(data){
			if(data==0){
            ...
            $('#productlist_'+id).parents('button').prev('button').attr('onclick','updateQuantityM('+key+', '+(element+1)+','+id+')');
            $('#productlist_'+id).parents('button').next('button').attr('onclick','updateQuantityP('+key+', '+(element+1)+','+id+')');
            
            
        });
    }
function updateQuantityM(key, element, id){
...
<?php /**PATH D:\xampp8.2.12\htdocs\bazarnao-v4-laravel-9\resources\views/frontend/layouts/app.blade.php ENDPATH**/ ?>